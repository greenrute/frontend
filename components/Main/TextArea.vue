...
<template>
  <div>
    <MainTag v-if="label" :id="id">
      {{ label }}
      <span class="inline-flex items-center rounded-md bg-gray-50 dark:bg-gray-400/10 px-2 py-1 text-xs font-medium text-gray-600 dark:text-gray-400 ring-1 ring-inset ring-gray-500/10 dark:ring-gray-400/20">{{ $t('optional') }}</span>
    </MainTag>
    <div class="relative rounded-md">
      <textarea :id="id" :type="type" :value="modelValue" v-bind="$attrs" @invalid="valid = false" ref="input"
