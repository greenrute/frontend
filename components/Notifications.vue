...
import { CheckCircleIcon, InformationCircleIcon, ExclamationTriangleIcon, ExclamationCircleIcon, BellIcon } from '@heroicons/vue/24/outline'
...
<template>
  <div aria-live="assertive" class="pointer-events-none fixed z-20 inset-0 flex items-end px-4 py-6 sm:items-start sm:p-6">
    <div class="flex w-full flex-col items-center space-y-4 sm:items-end">
      <transition v-for="notification in notifications.slice().reverse()" enter-active-class="transform ease-out duration-300 transition" enter-from-class="translate-y-2 opacity-0 sm:translate-y-0 sm:translate-x-2" enter-to-class="translate-y-0 opacity-100 sm:translate-x-0" leave-active-class="transition ease-in duration-100" leave-from-class="opacity-100" leave-to-class="opacity-0">
          ...
          <div class="p-4">
            <div class="flex items-start">
              ...
              <div class="ml-4 flex flex-shrink-0">
                <button type="button" @click="notification.close" class="inline-flex rounded-md bg-white text-gray-400 hover:text-gray-500 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2">
