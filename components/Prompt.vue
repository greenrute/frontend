            ...
            <DialogPanel class="relative transform overflow-hidden rounded-2xl bg-white px-4 pt-5 pb-4 text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg sm:p-6">
              <div>
                <div class="mx-auto flex h-12 w-12 items-center justify-center rounded-full bg-gray-100">
                  <QuestionMarkCircleIcon class="h-6 w-6 text-gray-600" aria-hidden="true" />
                </div>
                <div class="mt-3 text-center sm:mt-5">
                  <DialogTitle as="h3" class="text-lg font-medium leading-6 text-gray-900">{{ question.title }}</DialogTitle>
                  <div class="mt-2">
              ...
              <div class="mt-5 sm:mt-6 sm:grid sm:grid-flow-row-dense sm:grid-cols-2 sm:gap-3">
                <template v-if="question.options.length === 2">
                  <button type="button" class="relative inline-flex w-full justify-center rounded-full border border-transparent bg-green-600 px-4 py-2 text-base font-medium text-white shadow-sm hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 sm:col-start-2 sm:text-sm" @click="question.show = false; question.options[0].action()">
                    <component :is="icon(question.options[0].icon)" class="h-5 w-5 absolute left-3.5 top-[50%] -translate-y-[50%]" />
                    {{ question.options[0].title }}
                  </button>
                  <button type="button" class="relative mt-3 inline-flex w-full justify-center rounded-full border border-gray-300 bg-white px-4 py-2 text-base font-medium text-gray-700 shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 sm:col-start-1 sm:mt-0 sm:text-sm" @click="question.show = false; question.options[1].action()">
