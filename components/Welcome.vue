    ...
    <TransitionRoot as="template" :show="open.data">
      <Dialog as="div" class="relative z-10" @close="completeView">
        <TransitionChild as="template" enter="ease-out duration-300" enter-from="opacity-0" enter-to="opacity-100" leave="ease-in duration-200" leave-from="opacity-100" leave-to="opacity-0">
          <div class="fixed inset-0 bg-gray-500 dark:bg-zinc-700 bg-opacity-75 dark:bg-opacity-75 transition-opacity backdrop-blur" aria-hidden="true" />
        </TransitionChild>

        <div class="fixed inset-0 z-10 overflow-y-auto">
          <div class="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
            <TransitionChild as="template" enter="ease-out duration-300" enter-from="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" enter-to="opacity-100 translate-y-0 sm:scale-100" leave="ease-in duration-200" leave-from="opacity-100 translate-y-0 sm:scale-100" leave-to="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95">
              <DialogPanel id="welcome-wrap-el" class="relative transform overflow-hidden rounded-3xl bg-white px-4 pt-5 pb-4 text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg sm:p-6">
                <div ref="firstStage" class="transition-all h-max">
                  <div>
                    <div class="mx-auto flex h-12 w-12 items-center justify-center rounded-full bg-zinc-100">
                      <HandRaisedIcon class="h-6 w-6 text-zinc-600 motion-safe:animate-wave origin-bottom" aria-hidden="true" />
                    </div>
                    <div class="mt-3 text-center sm:mt-5">
                      <DialogTitle as="h3" class="text-lg font-medium leading-6 text-gray-900">Вітаємо!</DialogTitle>
                      <div class="mt-2">
                        <p class="text-sm text-gray-500">Ви успішно зареєструвалися в GreenRute, хочемо розказати Вам про деякі аспекти користування застосунком.</p>
                      </div>
                    </div>
                  </div>
                  <MainButton variant="solid" color="zinc" class="mt-5 sm:mt-6 w-full" @click="toSecondStage">
                    Далі&nbsp;
                ...
                <div ref="secondStage" class="transition-all h-max opacity-0 absolute inset-4 top-5 sm:inset-6 translate-x-full">
                  <div>
                    ...
                    <div class="mt-3 text-center sm:mt-5">
                      <DialogTitle as="h3" class="text-lg font-medium leading-6 text-gray-900">Клас — основна структурна ланка GreenRute</DialogTitle>
                      <div class="mt-2">
                        <p class="text-sm text-gray-500">GreenRute складається з класів. Ви можете їх знайти на панелі зліва &lt;-. Ви можете їх створювати та приєднуватись до класів. На головній сторінці Ви можете вибрати, який клас буде відображатись за замовчуванням.</p>
                      </div>
                    </div>
                  </div>
                  <MainButton variant="solid" color="zinc" class="mt-5 sm:mt-6 w-full" @click="toThirdStage">
                    Далі&nbsp;
                    <ArrowRightIcon class="h-4 w-4" />
                  </MainButton>
                </div>
                <div ref="thirdStage" class="transition-all h-max opacity-0 absolute inset-4 top-5 sm:inset-6 translate-x-[200%]">
                  <div>
                    ...
                    <div class="mt-3 text-center sm:mt-5">
                      <DialogTitle as="h3" class="text-lg font-medium leading-6 text-gray-900">Спільний розклад</DialogTitle>
                      <div class="mt-2">
                        <p class="text-sm text-gray-500">Всередині класів Ви можете мати спільний розклад та домашнє завдання. Ви можете керувати правами учасників і вибирати, хто може додавати завдання.</p>
                      </div>
                    </div>
                  </div>
                  <div class="mt-5 sm:mt-6 sm:grid sm:grid-flow-row-dense sm:grid-cols-2 sm:gap-3">
                    <MainButton variant="outline" color="zinc" class="w-full" @click="completeView">Закрити</MainButton>
                    <MainButton variant="solid" color="zinc" class="w-full mt-3 sm:mt-0">
                      Створити клас&nbsp;
