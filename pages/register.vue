...
<template>
  <Head>
    <Title>{{ $t('register', 0) }} - GreenRute</Title>
  </Head>
    ...
    <div class="relative z-10 flex flex-1 flex-col bg-white dark:bg-zinc-900 py-10 px-4 shadow-2xl sm:justify-center md:flex-none md:px-28">
      <div class="mx-auto w-full max-w-md sm:px-4 md:w-96 md:max-w-sm md:px-0">
        ...
        <MainForm @validated="login" class="mt-10 grid grid-cols-1 gap-y-8">
          <MainTextInput :label="$t('name')" :invalid="$t('the name must not be shorter than n characters', 3)" id="name" type="text" v-model="credentials.name" ref="name" autocomplete="name" required minlength="3" />
          ...
          <MainTextInput :label="$t('password')" :invalid="$t('the password must be at least n characters long', 3)" id="password" type="password" v-model="credentials.password" ref="password" autocomplete="new-password" required minlength="3" />
          <div>
