...
const googleSignUpButton: Ref<HTMLDivElement | null> = ref(null)

const name: Ref<{ input: HTMLInputElement } | null> = ref(null)
const email: Ref<{ input: HTMLInputElement } | null> = ref(null)
const password: Ref<{ input: HTMLInputElement } | null> = ref(null)

onMounted(() => {
  googleSignIn.render(googleSignUpButton, 'signup_with')

  if (credentials.name === '' && !!name.value) {
    credentials.name = name.value?.input.value
  }
...
<template>
  <Head>
    <Title>Реєстрація - GreenRute</Title>
  </Head>
        ...
        <div class="flex flex-col">
          ...
          <div class="mt-20">
            ...
            <p class="mt-2 text-sm text-gray-700">
              Вже зареєстровані?{{ ' ' }}
              <NuxtLink to="/login" class="font-medium text-green-600 hover:underline">Ввійдіть</NuxtLink>
              у свій акаунт
            </p>
          </div>
        </div>
        <MainForm @validated="login" class="mt-10 grid grid-cols-1 gap-y-8">
          <MainTextInput label="Ім'я" invalid="Ім'я має бути не коротшим, ніж 3 символа" id="name" type="text" v-model="credentials.name" ref="name" autocomplete="name" required minlength="3" />
          ...
          <MainTextInput label="Пароль" invalid="Пароль має бути не меншим, ніж 3 символа" id="password" type="password" v-model="credentials.password" ref="password" autocomplete="new-password" required minlength="3" />
          <div>
